 ...
 *  • ids 0 and 1 are Word's `separator` and `continuationSeparator`, never a real note. A new note
 *    starts at 2, and a part always carries the two separators even when it has no notes.
...
export const NOTE_SEPARATOR_ID = 0;
export const NOTE_CONTINUATION_ID = 1;
/** The first id a real note may use. */
 ...
 * The separators (ids 0 and 1) are skipped: they are structure, not notes.
 */
export function readNotesPart(xml: string, kind: NoteKind): PackagedNote[] {
  ...
  while ((match = element.exec(xml))) {
    ...
    if (!Number.isFinite(id) || id < FIRST_NOTE_ID) continue;
    const body = match[3] ?? '';
    const full = match[0];
...
/** The pair of separator elements Word requires, with ids 0 and 1. */
function separatorsXml(kind: NoteKind): string {
  const tag = ELEMENT[kind];
  return `<w:${tag} w:type="separator" w:id="${NOTE_SEPARATOR_ID}"><w:p><w:r><w:separator/></w:r></w:p></w:${tag}>`
    + `<w:${tag} w:type="continuationSeparator" w:id="${NOTE_CONTINUATION_ID}"><w:p><w:r><w:continuationSeparator/></w:r></w:p></w:${tag}>`;
}
