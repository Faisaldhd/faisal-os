...
- **الموقع المباشر:** <https://faisal-os.pages.dev/> — يُنشر مع تطبيق سطح المكتب معاً: بعد الدمج في `main` يُبنى إصدار التطبيق أولاً، وعند نجاحه فقط يُنشر الويب من نفس النسخة.
...
<https://faisal-os.pages.dev/> — published together with the desktop app: a merge to `main` builds and releases the desktop app first (`.github/workflows/desktop-release.yml`), and only when that succeeds does the same workflow publish the web (Cloudflare Pages via the `release` branch, and GitHub Pages).
...
Each app has a static manifest and a code + CSS chunk that loads on first launch: Files, Terminal, Text Editor, Browser, Faisal AI, Calculator, Image Viewer, Clock, System Monitor, Store, Settings, Vault and the Streamed Browser. See the [apps table](#apps) for ids, permissions and instance behaviour.

### Reaching external sites

Three honest paths, in order of how much they can show:

- **Embedded Web Apps** (`src/apps/web/`): a packaged site opens as a normal OS window. Every wired site was measured for `X-Frame-Options` / `frame-ancestors` before it was added, and the ones that refuse framing (Google, YouTube, DuckDuckGo) are labelled as such instead of showing a blank frame. Sites that do embed get a real window with a toolbar, back/forward, reload, multi-instance and a remembered address per site. Also here: a YouTube embed player that accepts any watch/shorts/youtu.be link, and a Search app with a keyless Wikipedia provider (the optional Google Programmable Search provider is present but inert — the owner declined the CSP widening it needs).
- **Local proxy** (`tools/local-proxy.mjs`, OPT-IN, off by default): a loopback-only Node process the owner runs himself, with a token and an SSRF guard on the target and on every redirect hop. It removes `X-Frame-Options` and CSP `frame-ancestors` — nothing else — so a site that merely refuses framing can be read (reader mode, text rendered natively by the OS) or shown raw. An iframe cannot send the token header, so raw mode loads `/view?ticket=…` with a single-use 60-second ticket that is dead before the framed page's own scripts run. See [docs/LOCAL_PROXY.md](docs/LOCAL_PROXY.md).
- **Streamed browser** (`src/apps/stream/`): a real Chromium in a Docker container the owner runs, its screen framed into an OS window — so Google and YouTube work fully, because the container's browser opens them as top-level pages and this page never frames them. Needs Docker and is never claimed to be connected when it is not. See [docs/STREAM_BROWSER.md](docs/STREAM_BROWSER.md).
